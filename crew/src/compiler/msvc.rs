        ...
        while let Some(handle) = set.join_next().await {
            match handle {
                Ok((addr, dispatched, output)) => {
                    ...
                    log::debug!("dist compile project: {:?} addr: {:?} output: {:?} error: {:?}", project_, &addr, String::from_utf8_lossy(&output.out), String::from_utf8_lossy(&output.err));
                    {
                    ...
                    if !sources.is_empty() {
                        ...
                        (_, index, left, sources) = self.sender.lock().unwrap().schedule_for_sources(Some(addr), &sources, Some(0));
        ...
        for line in files {
            ...
            if line.ends_with(".cpp") || line.ends_with(".c") || line.ends_with(".cc") || line.ends_with(".cxx") || line.ends_with(".i") {
                if sync_compile_result {
                    let mut obj: Option<(std::ffi::OsString, i64, bytes::Bytes, bool)> = None;
                    ...
                    match std::fs::File::open(&result_path) {
                        Ok(file) => {
                            ...
                            obj = Some((result_path.clone().into_os_string(), -1, bytes::Bytes::from(contents), true));
                        },
                        Err(error) => {
                    ...
                    if last == Some(&line)
                    {
                        ...
                        match std::fs::File::open(&result_path) {
                            Ok(file) => {
                                ...
                                let mut file = std::io::BufReader::new(file);
                                let _ = file.read_to_end(&mut contents).unwrap();
                                pdb = Some((result_path.clone().into_os_string(), -1, bytes::Bytes::from(contents), true));
                            },
                            Err(error) => {
                                if error.kind() == std::io::ErrorKind::NotFound {
                        ...
                        match std::fs::File::open(&result_path) {
                            Ok(file) => {
                                ...
                                let mut file = std::io::BufReader::new(file);
                                let _ = file.read_to_end(&mut contents).unwrap();
                                idb = Some((result_path.into_os_string(), -1, bytes::Bytes::from(contents), true));
                            },
                            Err(error) => {
                                if error.kind() == std::io::ErrorKind::NotFound {
...
struct CompileAction {
    ...
    pub include_dir: Vec<std::path::PathBuf>,
...
fn parse_action_from_commands(build_and_compiler_type: &std::ffi::OsString, compiler_commands: &Vec<std::ffi::OsString>, working_dir: &std::ffi::OsString) -> CompileAction {
    ...
    let mut include_dir: Vec<std::path::PathBuf> = Vec::new();
        ...
        for (command, value) in compiler_commands.iter().zip(next) {
            ...
            if command == "/P" {
                ...
                if let Some(index) = value.to_string_lossy().find("=") {
                    let key = value.to_string_lossy()[..index].to_string();
                    let value = value.to_string_lossy()[index + 1..].to_string();
                    defines.insert(key, Some(value));
                }
                else {
                    defines.insert(value.to_string_lossy().to_string(), None);
                }
            }
